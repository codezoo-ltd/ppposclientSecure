...
int start_ssl_client(sslclient_context *ssl_client, const char *host,
  ...
  srv = ip_addr.u_addr.ip4.addr;

  fcntl(ssl_client->socket, F_SETFL,
        fcntl(ssl_client->socket, F_GETFL, 0) | O_NONBLOCK);
  ...
  int res = lwip_connect(ssl_client->socket, (struct sockaddr *)&serv_addr,
                         sizeof(serv_addr));
  if (res < 0 && errno != EINPROGRESS) {
    log_e("connect on fd %d, errno: %d, \"%s\"", ssl_client->socket, errno,
          strerror(errno));
    close(ssl_client->socket);
    return -1;
  }

  res = select(ssl_client->socket + 1, nullptr, &fdset, nullptr,
               timeout < 0 ? nullptr : &tv);
  if (res < 0) {
    log_e("select on fd %d, errno: %d, \"%s\"", ssl_client->socket, errno,
          strerror(errno));
    close(ssl_client->socket);
    return -1;
  } else if (res == 0) {
    log_i("select returned due to timeout %d ms for fd %d", timeout,
          ssl_client->socket);
    close(ssl_client->socket);
    return -1;
  } else {
    ...
    if (res < 0) {
      log_e("getsockopt on fd %d, errno: %d, \"%s\"", ssl_client->socket, errno,
            strerror(errno));
      close(ssl_client->socket);
      return -1;
    }
    ...
    if (sockerr != 0) {
      log_e("socket error on fd %d, errno: %d, \"%s\"", ssl_client->socket,
...
#define ROE(x, msg)                                                            \
  ...
  ROE(lwip_setsockopt(ssl_client->socket, SOL_SOCKET, SO_RCVTIMEO, &tv,
                      sizeof(tv)),
      "SO_RCVTIMEO");
  ROE(lwip_setsockopt(ssl_client->socket, SOL_SOCKET, SO_SNDTIMEO, &tv,
                      sizeof(tv)),
      "SO_SNDTIMEO");

  ROE(lwip_setsockopt(ssl_client->socket, IPPROTO_TCP, TCP_NODELAY, &enable,
                      sizeof(enable)),
      "TCP_NODELAY");
  ROE(lwip_setsockopt(ssl_client->socket, SOL_SOCKET, SO_KEEPALIVE, &enable,
                      sizeof(enable)),
      "SO_KEEPALIVE");
  ...
  if ((ret = mbedtls_ssl_config_defaults(
           &ssl_client->ssl_conf, MBEDTLS_SSL_IS_CLIENT,
  ...
  // MBEDTLS_SSL_VERIFY_REQUIRED if a CA certificate is defined on Arduino IDE
  // and MBEDTLS_SSL_VERIFY_NONE if not.
  ...
  if (insecure) {
    ...
    if (ret < 0) {
      // free the ca_cert in the case parse failed, otherwise, the old ca_cert
      // still in the heap memory, that lead to "out of memory" crash.
  ...
  if (!insecure && cli_cert != NULL && cli_key != NULL) {
    ...
    ret = mbedtls_x509_crt_parse(&ssl_client->client_cert,
                                 ...
                                 strlen(cli_cert) + 1);
    if (ret < 0) {
      // free the client_cert in the case parse failed, otherwise, the old
      // client_cert still in the heap memory, that lead to "out of memory"
      // crash.
    ...
    mbedtls_ssl_conf_own_cert(&ssl_client->ssl_conf, &ssl_client->client_cert,
                              &ssl_client->client_key);
  }
  ...
  if ((ret = mbedtls_ssl_setup(&ssl_client->ssl_ctx, &ssl_client->ssl_conf)) !=
      0) {
    return handle_error(ret);
  }

  mbedtls_ssl_set_bio(&ssl_client->ssl_ctx, &ssl_client->socket,
...
void stop_ssl_socket(sslclient_context *ssl_client, const char *rootCABuff,
...
int send_ssl_data(sslclient_context *ssl_client, const uint8_t *data,
                  size_t len) {
...
// Verifies certificate provided by the peer to match specified SHA256
// fingerprint
bool verify_ssl_fingerprint(sslclient_context *ssl_client, const char *fp,
...
bool verify_ssl_dn(sslclient_context *ssl_client, const char *domain_name) {
  ...
  std::transform(domain_name_str.begin(), domain_name_str.end(),
  ...
  while (common_name != nullptr) {
    ...
    if (!MBEDTLS_OID_CMP(MBEDTLS_OID_AT_CN, &common_name->oid)) {
      std::string common_name_str((const char *)common_name->val.p,
                                  common_name->val.len);
